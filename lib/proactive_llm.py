"""Shared LLM call path for proactive Discord messages (greeting, goodnight, outreach)."""
...
def salvage_proactive_from_thinking(raw: str) -> str:
    ...
    body = re.sub(
        r"(?is)</(?:redacted_thinking|thinking)>\s*",
        "",
        body,
    ).strip()
...
def resolve_proactive_provider(system, provider_key: str, model_name: str):
    llm = system.llm_chat
    if provider_key and model_name:
        ...
        return provider_key, provider, gen_params
...
def run_proactive_llm(
    ...
    *,
    ...
    if not system or not getattr(system, "llm_chat", None):
        logger.warning(f"[LEONA-DISCORD] {log_label} LLM: no system.llm_chat available")
        return ""

    from plugins.leona_discord.lib.bot_identity import bot_identity_fields

    fields = bot_identity_fields(account) if account else {}
    try:
        provider_key, provider, base_gen = resolve_proactive_provider(
            system, provider_key, model_name,
        )
        if not provider:
            logger.warning(
                f"[LEONA-DISCORD] {log_label} model {provider_key}/{model_name} unavailable"
            )
            return ""
        ...
        if last_raw and last_raw.strip():
            ...
            logger.warning(
                f"[LEONA-DISCORD] {log_label} LLM unusable output for {account or 'bot'} "
                f"({len(last_raw)} chars, likely thinking-only or truncated): {preview!r}"
            )
        return ""
    except Exception as e:
        logger.warning(f"[LEONA-DISCORD] {log_label} LLM failed: {e}")
        return ""

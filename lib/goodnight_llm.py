...
def generate_goodnight(
    ...
    *,
    ...
    if not system or not getattr(system, "llm_chat", None):
        logger.warning("[LEONA-DISCORD] Goodnight LLM: no system.llm_chat available")
        return ""
    ...
    identity_hint = build_proactive_post_hint(fields, purpose="goodnight")
    ...
    try:
        ...
        if provider_key and model_name:
            ...
            if not provider:
                logger.warning(f"[LEONA-DISCORD] Goodnight model {provider_key}/{model_name} unavailable")
                return ""
        ...
        logger.warning(f"[LEONA-DISCORD] Goodnight LLM failed: {e}")
        return ""
